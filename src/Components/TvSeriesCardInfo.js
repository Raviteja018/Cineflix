...
const TvSeriesCardInfo = () => {
  ...
  const movie = useSelector((store) => store.details?.tvSeriesCardDetails);
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-black via-gray-900 to-black text-white">
      ...
      <header className="flex items-center justify-between px-6 py-4 bg-black bg-opacity-80 shadow-md sticky top-0 z-50">
        <img src={logo} alt="img" className=" w-36 text-2xl font-bold text-red-600" />
        <button
          onClick={() => navigate("/")}
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 p-6 md:p-12">
        ...
        <div className="md:col-span-2 flex flex-col justify-center">
          <h1 className="text-4xl font-extrabold mb-4">{movie?.original_name}</h1>
          <p className="text-gray-300 mb-4">{movie?.overview.substring(0,300)}</p>
